...
def plot_data(ax, x, y, target_date):
  dates = [ get_date_before(target_date, i) for i in x ]
  ax.plot(dates, y, 'xk', ms=4, markeredgewidth=2, label='reported deaths')
...
def plot_model(ax, data_mean, data_abv, data_bel, target_date, c='k'):
  dates = [ get_date_before(target_date, -x) for x in [ x for x in range(data_mean.shape[0]) ][::-1] ]
