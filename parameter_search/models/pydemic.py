...
class PydemicModel:
    ...
    def update_model_params(self, model_params):
        ...
        if 'start_day' in model_params:
            self.start_date = self.get_date_tuple(date(2020,1,1) + timedelta(model_params['start_day']))
        if 'end_day' in model_params:
            self.end_day[2] = self.get_date_tuple(date(2020,1,1) + timedelta(model_params['end_day']))
        # update epidemiology
